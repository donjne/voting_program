...
describe("election", () => {
  ...
  it("applies and registers as a candidate (user 1)", async() => {
  ...
  it("applies and registers as a candidate (user 2)", async() => {
  ...
  it("closes election and declares final winners", async() => {
    ...
    assert.equal(electionData.winners.length, '1');
    assert.equal(electionData.winners[0].toNumber(), '1');
  });
});
